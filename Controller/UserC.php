<?php
require_once 'D:\Esprit 2eme\progs\xampp\htdocs\Unify_SocialNetwork-aziz\Config.php';
class UserC
{
    ...
    function addUser($User)
{
    ...
    public function updateuser($User, $id)
    {
        try {
            $db = config::getConnexion();
            $query = $db->prepare(
                'UPDATE user SET Nme = :Nme,Lname = :Lname,Email = :Email,Username = :Username,Pwd = :Pwd,Adress = :Adress,University = :University,classe = :classe
        WHERE  Id_User = :id'
            );
            $query->execute([
                'id' => $id,
